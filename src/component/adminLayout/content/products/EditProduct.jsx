...
function EditProduct() {
    ...
    const formik = useFormik({
        ...
        validate: (values) => {
            ...
            const detailsErrors = values.details.map((item) => {
                ...
                if (!item.key) {
                    detailError.key = "Please enter size";
                } else if (!/^[a-zA-Z]+$/.test(item.key)) {
                    detailError.key = "Please enter only letters";
                }

                if (!item.value) {
                    detailError.value = "Please enter price";
                } else if (!/^[0-9]+$/.test(item.value)) {
                    detailError.value = "Please enter only numbers";
                }

                if (!item.qty) {
                    detailError.qty = "Please enter quantity";
                } else if (!/^[0-9]+$/.test(item.qty)) {
                    detailError.qty = "Please enter only numbers";
                }
        ...
        onSubmit: async (values) => {
            try {
                await axios.put(`https://684fcb12e7c42cfd1795faf8.mockapi.io/adminpannelproject/products/${id}`, values);
    ...
    const getProductInfo = async () => {
        try {
            ...
            formik.setValues({
                ...productData.data,
                details: productData.data.details?.length ? productData.data.details : [{ key: "", value: "", qty: "" }]
            });
        } catch (error) {
            console.error(error);
        }
                            ...
                            <div key={index} className="flex flex-col md:flex-row md:space-x-6 space-y-4 md:space-y-0">
                                ...
                                <div className="flex flex-col w-full md:w-1/3">
                                    ...
                                    {formik.touched.details?.[index]?.key &&
                                        Array.isArray(formik.errors.details) &&
